...
class FontWidget(QtWidgets.QPushButton):
    def __init__(self, text: str, font: QtGui.QFont) -> None:
        super().__init__(
            ...
            clicked=self._on_click)  # type: ignore
...
def add_default(form: QtWidgets.QFormLayout,
                label: str,
    ...
    layout.addWidget(QtWidgets.QPushButton(
        tr("Default"), flat=True, clicked=default_callback))  # type: ignore
...
def create_setting_dialog(parent: QtWidgets.QWidget) -> None:
    ...
    click_ms = QtWidgets.QSpinBox(minimum=0, maximum=1000, value=settings.click_ms)
    ...
    form.addWidget(QtWidgets.QLabel(
        tr("Time between press and release with possible mouse movements that will be registered as single mouse click"),
        wordWrap=True
    ))
