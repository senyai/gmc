...
def _nicenum(x):
    ...
    return floor(x / expv10 * 2. + 0.5) * .5 * expv10


class VideoWidget(QtWidgets.QWidget):
    pixmap_changed = QtCore.pyqtSignal(QtGui.QPixmap,
                                       name='pixmap_changed')
    before_frame_load = QtCore.pyqtSignal(int, QtWidgets.QGraphicsScene,
                                          name='before_frame_load')
    after_frame_load = QtCore.pyqtSignal(int, QtWidgets.QGraphicsScene,
                                         name='after_frame_load')
    _frame_number = None
    ...
    def __init__(self, default_actions):
        ...
        self._seek_slider = SeekSlider(
            limit_widget=self._view,
            valueChanged=self._seek)
        ...
        self._play_action = QtWidgets.QAction(
            get_icon("play"), "Play", self, checkable=True,
            toggled=self._play_toggled)
        seek_toolbar.addAction(self._play_action)
        seek_toolbar.addAction(QtWidgets.QAction(
    ...
    def _play_toggled(self, play):
        if play:
            self._seek_slider.setRepeatAction(
                QtWidgets.QSlider.SliderSingleStepAdd, 20, 1.0 / self._fps * 500)
        else:
            self._seek_slider.setRepeatAction(
    ...
    def add_markup_action(self, name, shortcut, icon, markup_object, **kwargs):
        action = QtWidgets.QAction(
            get_icon(icon), "{} ({})".format(name, shortcut), self,
            triggered=lambda: self._set_markup_object(markup_object),
            shortcut=shortcut, checkable=True, **kwargs)
    ...
    def add_default_actions(self):
        next_frame_action = QtWidgets.QAction(
            get_icon('next_frame'), "Next Frame (F)", self,
            triggered=self.next_frame, shortcut='F')
        next_second_action_ = QtWidgets.QAction(
            get_icon('next_second'), "Next Second (Shift+F)", self,
            triggered=lambda: self._go(self._fps), shortcut='Shift+F')

        previous_frame_action = QtWidgets.QAction(
            get_icon('prev_frame'), "Previous Frame (B)", self,
            triggered=lambda: self._go(-1), shortcut='B')
        previous_second_action = QtWidgets.QAction(
            get_icon('prev_second'), "Previous Second (Shift+B)", self,
            triggered=lambda: self._go(-self._fps), shortcut='Shift+B')

        nav = (next_frame_action, next_second_action_,
               previous_frame_action, previous_second_action)

        for actions in (self._view.get_zoom_actions(), nav,
                        (self._view.delete_action,), self._default_actions):
    ...
    def open_file(self, movie_filename):
        ...
        min_width = metrics.width(
            "9" * len(str(frames_count))) + self._frame_label.margin() * 2
